...
## Modified MB-iSTFT-VITS incorporating F0.
## F0 MB-iSTFT-VITS was written by ChatGPT so I don't know the mathematical details of how it works...

class F0Predictor(nn.Module):
    """
    Prediction a pitch(F0) using z/m_p.
  
    input: [B, C, T] (channel-first like other modules)
    output: [B, 1, T] (raw f0 values, arbitrary scale)
    """
    def __init__(self, in_channels, hidden_channels=128, kernel_size=3, p_dropout=0.1):
        super().__init__()
        pad = kernel_size // 2
    ...
    def forward(self, x, x_mask=None):
        # x: [B, C, T]
        h = self.conv1(x * (x_mask if x_mask is not None else 1.0))
        h = self.act(h)
        h = self.drop(h)
        h = self.conv2(h * (x_mask if x_mask is not None else 1.0))
        h = self.act(h)
        h = self.drop(h)
        f0 = self.proj(h)  # [B, 1, T]
...
class LanguageEmbedding(nn.Module):
  def __init__(self, num_langs, dim):
    super().__init__()
    self.embedding = nn.Embedding(num_langs, dim)

  def forward(self, lang_id, length):
    return self.embedding(lang_id).unsqueeze(1).expand(-1, length, -1)


class ReferenceEncoder(nn.Module):
  def __init__(self, mel_dim=80, hidden=128):
    super().__init__()
    self.net = nn.Sequential(
  ...
  def forward(self, mel):
    # mel: [B, T, 80]
    return self.net(mel.mean(dim=1)) # [B, H]


class MASAlignment:
  def __call__(self, log_p, dur):
    # TODO: replace with real MAS
    return torch.repeat_interleave(log_p, dur.squeeze(-1).long(), dim=1)
...
class MultiPeriodDiscriminator(torch.nn.Module):
    def __init__(self, use_spectral_norm=False):
        ...
        periods = [2,3,5,7] # [2,3,5,7,11] -> [2,3,5,7]
...
class SynthesizerTrn(nn.Module):
  ...
  def __init__(self, 
    ...
    **kwargs
    ...
    # languages embedding
    # please input a supported languages numbers. now supported languages: en, ch, ja, ko = 4
    self.num_langs = 4
    self.lang_embedding = nn.Embedding(self.num_langs, self.inter_channels)

    # reference encoder
    self.use_ref_encoder = True
    if self.use_ref_encoder:
        self.ref_enc = ReferenceEncoder(mel_dim=spec_channels, hidden=128)
        self.ref_proj = nn.Linear(128, self.inter_channels)
    ...
    self.use_acoustic_predictor = True
    if self.use_acoustic_predictor:
        self.f0_pred = F0Predictor(self.inter_channels, hidden_channels=max(64, self.inter_channels//4))
        self.f0_to_z = nn.Conv1d(1, self.inter_channels, 1)
        nn.init.zeros_(self.f0_to_z.weight)
        nn.init.zeros_(self.f0_to_z.bias)

  def forward(self, x, x_lengths, y, y_lengths, lang_id=None, ref_mel=None, sid=None):
    x, m_p, logs_p, x_mask = self.enc_p(x, x_lengths)

    # languages embedding
    if lang_id is not None:
      lang_emb = self.lang_embedding(lang_id).unsqueeze(-1)  # [B, C, 1]
      lang_emb = lang_emb * x_mask  # broadcast
      m_p = m_p + lang_emb
      logs_p = logs_p

    # reference encoder
    if self.use_ref_encoder and ref_mel is not None:
        ref_vec = self.ref_enc(ref_mel)      # [B, H]
        ref_vec = self.ref_proj(ref_vec)     # [B, C]
        ref_vec = ref_vec.unsqueeze(-1)      # [B, C, 1]
        m_p = m_p + ref_vec # add to encoder  
    ...
    if self.use_acoustic_predictor:
        f0_raw = self.f0_pred(z, x_mask=y_mask)
        f0_emb = self.f0_to_z(f0_raw) * y_mask
        z_p = z_p + f0_emb
    ...
    if self.use_acoustic_predictor:
        f0_slice, _ = commons.rand_slice_segments(f0_emb, y_lengths, self.segment_size)
        z_slice = z_slice + f0_slice
        
    # encoder
    o, o_mb = self.dec(z_slice, g=g)
    return o, o_mb, l_length, attn, ids_slice, x_mask, y_mask, (z, z_p, m_p, logs_p, m_q, logs_q), f0_raw

  def infer(self, x, x_lengths, lang_id=None, ref_mel=None, sid=None, noise_scale=1, length_scale=1, noise_scale_w=1., max_len=None, f0_external=None):
    x, m_p, logs_p, x_mask = self.enc_p(x, x_lengths)

    # languages embedding
    if lang_id is not None:
      lang_emb = self.lang_embedding(lang_id).unsqueeze(-1)
      lang_emb = lang_emb * x_mask
      m_p = m_p + lang_emb

    # reference encoder
    if self.use_ref_encoder and ref_mel is not None:
        ref_vec = self.ref_enc(ref_mel)
        ref_vec = self.ref_proj(ref_vec).unsqueeze(-1)
        m_p = m_p + ref_vec
    ...
    if f0_external is not None:
        f0_raw = f0_external
    elif self.use_acoustic_predictor:
        f0_raw = self.f0_pred(m_p, x_mask=y_mask)
    else:
        f0_raw = None
  ...
  def voice_conversion(self, y, y_lengths, sid_src, sid_tgt):
    ...
    if self.use_acoustic_predictor:
        f0_raw = self.f0_pred(z, x_mask=y_mask)  # [B,1,T]
        f0_emb = self.f0_to_z(f0_raw) * y_mask
        z_p = z_p + f0_emb
